...
allspikes = [];
for n=1:NumChannels
    allspikes = [allspikes;clean_spikes{n}];         
end


[exactcounts, uniquespikes] = hist(allspikes,unique(allspikes));

mask_events = false(max(uniquespikes),1);

eth=5;

for i=1:length(uniquespikes)
    	if(exactcounts(i)<=eth)
            mask_events(uniquespikes(i))=1;
        end
end
...
for n=1:NumChannels
    ...
    cs=double(active_data.frameno(channelSpikes));
    clean_spikes{n} = cs(mask_events(cs)) 
...
save('../Data/data_spikeTimes_for_TE_clean.mat', 'data'); 
Calculate_TE('../Data/',"data_spikeTimes_for_TE_clean",'../Results/',10,20);

NullModel('../Data/','../Null_model/',"data_spikeTimes_for_TE_clean");
Significance_Test('../Results/','../Null_model/','../Data',"data_spikeTimes_for_TE_clean",1000,0.05);
...
spikeTimes = dlmread('../Results/data_spikeTimes_for_TE_clean_TEPk.txt');
spikeTimes_delay = dlmread('../Results/data_spikeTimes_for_TE_clean_TEdelays.txt');
distance_mat = dlmread('../Results/data_spikeTimes_for_TE_clean_DistanceMat.txt');
...
save('../Data/combined_active_data_clean.mat', '-struct', 'data1');
...
spikeTimes = dlmread('../Data/data_spikeTimes_for_TE_clean_TEPk.txt');
P_values = dlmread('../Data/data_spikeTimes_for_TE_clean_Pvalues.txt');
...
% Save the combined data to a new .mat file
save('../Data/Significant_data_clean.mat', '-struct', 'data2');
